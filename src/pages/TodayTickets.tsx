...
export function TodayTicketsPage() {
  ...
  const categories = useMemo(() => [...new Set(todayTickets.map((t) => t.category).filter(Boolean))].sort(), [todayTickets]);

  const filtered = useMemo(() => {
    let list = categoryFilter ? todayTickets.filter((t) => t.category === categoryFilter) : todayTickets;
    list = [...list].sort((a, b) => {
    ...
    return list;
  }, [todayTickets, categoryFilter, sortBy]);
            ...
            {filtered.length === 0 && (
              <tr>
                <td colSpan={6} style={{ padding: 24, textAlign: 'center', color: 'var(--color-text-muted)' }}>
                  {todayTickets.length === 0 ? 'Dnes zatiaľ žiadne tikety.' : 'Žiadne tikety nezodpovedajú filtru.'}
                </td>
              </tr>
            )}
            {filtered.map((t) => (
