...
export function KbArticlesSettingsPage({ embedded = false }: { embedded?: boolean } = {}) {
  ...
  async function handleAdd(e: FormEvent) {
    ...
    await createKbArticle({ title: form.title.trim(), category: form.category.trim(), url: form.url.trim(), order });
    setForm(emptyForm());
  }

  function startEdit(a: KbArticle) {
    setEditingId(a.id);
    setEditDraft({ title: a.title, category: a.category, url: a.url });
  }
